...
> Если стек поднимается на этой машине впервые (нет `.env`, не запускался `bootstrap.sh`, нет значения в переменной) — идти в раздел «Подготовка с нуля» ниже, потом возвращаться сюда.

Поднять стек (идемпотентно: уже запущенные контейнеры не трогает):
...
Эта последовательность приводит систему в состояние, годное для записи — нужна при первом подъёме на машине, а также после любого рестарта Conjur-контейнера (Postgres под Conjur не имеет персистентного volume, состояние теряется).
