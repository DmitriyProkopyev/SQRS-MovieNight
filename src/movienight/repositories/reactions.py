...
class ReactionRepository:
    ...
    def delete(self, reaction: FoodReaction) -> None:
        self.db.execute(delete(FoodReaction).where(FoodReaction.id == reaction.id))
        self.db.commit()

    def count_for_proposal_and_category(self, proposal_id: int, category: str) -> int:
    ...
    def counts_for_proposal_ids(self, proposal_ids: list[int]) -> dict[int, dict[str, int]]:
        if not proposal_ids:
            return {}
        statement = (
            select(FoodReaction.proposal_id, FoodReaction.category, func.count(FoodReaction.id))
            .where(FoodReaction.proposal_id.in_(proposal_ids))
            .group_by(FoodReaction.proposal_id, FoodReaction.category)
        )
    ...
    def user_categories_for_proposal_ids(self, user_id: int, proposal_ids: list[int]) -> dict[int, list[str]]:
        if not proposal_ids:
            return {}
        statement = select(FoodReaction).where(
            FoodReaction.user_id == user_id,
        ...
        for item in self.db.scalars(statement).all():
            data.setdefault(item.proposal_id, []).append(str(item.category))
        return data
