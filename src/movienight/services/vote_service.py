...
class VoteService:
    ...
    def add_vote(
        ...
        component = build_conflict_component(
            proposal,
            self.proposals.list_by_room(proposal.room)
        )
        component_ids = [item.id for item in component]
        user_votes = self.votes.get_user_votes_in_group(
            current_user.id,
            component_ids
        )
        already_voted_for_target = self.votes.exists_for_user_and_proposal(
            current_user.id,
            proposal_id
        )
        ensure_vote_allowed(
    ...
    def remove_vote(
        ...
        proposal = self._require_proposal(proposal_id)
        vote = self.votes.find_by_user_and_proposal(
            current_user.id,
            proposal_id
        )
        ensure_vote_removal_allowed(
